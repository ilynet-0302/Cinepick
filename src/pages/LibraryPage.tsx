...
export default function LibraryPage() {
  ...
  const {
    ...
    queryKey: ["library-titles", tab, keys],
    queryFn: () => getTitlesByKeys(keys),
    ...
    () =>
      [...data].sort((a, b) =>
        sort === "rating"
          ...
          : sort === "year"
            ? b.year - a.year
            : keys.indexOf(mediaKey(b)) - keys.indexOf(mediaKey(a)),
      ),
    [data, keys, sort],
  );

  return (
    <div className="standard-page library-page page-enter">
      <div className="page-title-row">
        <div>
          <p className="eyebrow">All your stories</p>
          <h1>My library</h1>
          <p>Everything you want to watch, loved, and finished.</p>
        </div>
        <div className="library-summary">
          <span>
            <b>{library.watchlist.length}</b> queued
          </span>
          <span>
            <b>{library.watched.length}</b> watched
          </span>
        </div>
      </div>
      {(library.cloudLoading || library.syncError) && (
        <div className={`sync-banner ${library.syncError ? "error" : ""}`}>
          <span />
          {library.syncError || "Synchronizing your library…"}
        </div>
      )}
        ...
        <div className="library-tabs">
          {tabs.map(({ value, label, icon: Icon }) => (
            ...
            >
              <Icon size={17} />
              {label}
              <span>{library[value].length}</span>
            </button>
          ))}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="empty-state">
          ...
          <h2>Your {tab} is waiting</h2>
          <p>Explore TMDB and use the quick actions on any title.</p>
