...
const catalog: Movie[] = Array.from({ length: 12 }, (_, index) => {
  ...
  return {
    ...
    type: "movie",
    year: 2020 + (index % 5),
    rating: 7 + (index % 4) * 0.3,
    voteCount: 500,
    genres: genreSets[group],
    genreIds: [],
    language: "en",
...
describe("recommendation engine", () => {
  it("changes recommendations when the requested mood changes", () => {
    const funny = rankRecommendations(catalog, criteria).map(
      (movie) => movie.id,
    );
    const cerebral = rankRecommendations(catalog, {
      ...criteria,
      moodGenres: ["Sci-Fi", "Mystery"],
    }).map((movie) => movie.id);
    expect(cerebral).not.toEqual(funny);
  });

  it("respects runtime preferences when matching titles exist", () => {
    const long = rankRecommendations(catalog, {
      ...criteria,
      moodGenres: [],
      time: "long",
    });
  ...
  it("returns a fresh set when previous picks are excluded", () => {
    ...
    const next = rankRecommendations(catalog, {
      ...criteria,
      excludedIds: first.map((movie) => movie.id),
      seed: 1,
    });
    expect(
      next.some((movie) => first.some((pick) => pick.id === movie.id)),
    ).toBe(false);
  });
});
