...
export function scoreMovie(movie: Movie, criteria: PickCriteria) {
  ...
  const moodMatches = movie.genres.filter((genre) =>
    criteria.moodGenres.includes(genre),
  ).length;
  const companyMatches = movie.genres.filter((genre) =>
    criteria.companyGenres.includes(genre),
  ).length;

  score += moodMatches ? 13 + (moodMatches - 1) * 4 : -4;
  if (criteria.companyGenres.length)
    score += companyMatches ? 7 + (companyMatches - 1) * 2 : -2;
  if (criteria.preferredGenre !== "All")
    score += movie.genres.includes(criteria.preferredGenre) ? 18 : -14;
  if (criteria.time) score += timeMatches(movie, criteria.time) ? 10 : -9;
  if (movie.rating >= criteria.minRating) score += 4;
  else score -= 10 + (criteria.minRating - movie.rating) * 12;
...
export function rankRecommendations(
  ...
  const available = candidates.filter(
    (movie) =>
      movie.type === "movie" &&
      (criteria.includeWatched || !criteria.watchedIds.includes(movie.id)),
  );
  const ranked = available.map((movie) => ({
    movie,
    score: scoreMovie(movie, criteria),
  }));
  const selected: Movie[] = [];

  while (selected.length < limit && ranked.length) {
    ranked.sort((a, b) => {
      const diversityPenaltyA = selected.reduce(
        (penalty, pick) =>
          penalty +
          a.movie.genres.filter((genre) => pick.genres.includes(genre)).length *
            2.5,
        0,
      );
      const diversityPenaltyB = selected.reduce(
        (penalty, pick) =>
          penalty +
          b.movie.genres.filter((genre) => pick.genres.includes(genre)).length *
            2.5,
        0,
      );
...
export function getMatchReasons(movie: Movie, criteria: PickCriteria) {
  ...
  const moodGenre = movie.genres.find((genre) =>
    criteria.moodGenres.includes(genre),
  );
  const companyGenre = movie.genres.find((genre) =>
    criteria.companyGenres.includes(genre),
  );
  ...
  if (
    ...
    movie.genres.includes(criteria.preferredGenre)
  )
    reasons.push(criteria.preferredGenre);
  if (companyGenre && companyGenre !== moodGenre)
    reasons.push(`Great for ${companyGenre.toLowerCase()} fans`);
